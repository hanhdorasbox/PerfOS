...
function MarkdownText({ content }: { content: string }) {
  ...
  while (i < lines.length) {
    ...
    if (/^```/.test(line)) {
      ...
      elements.push(
        <div key={`code-${i}`} style={{ margin: '6px 0', borderRadius: 8, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.1)' }}>
          ...
          <pre style={{ margin: 0, padding: '10px 12px', background: 'rgba(255,255,255,0.04)', overflowX: 'auto', fontSize: 11, color: '#B8A4FF', lineHeight: 1.6, fontFamily: 'monospace' }}>
            {codeLines.join('\n')}
          </pre>
        </div>
      )
    ...
    if (/^###\s+/.test(line)) {
      elements.push(
        <div key={i} style={{ fontSize: 12, fontWeight: 700, color: '#B8A4FF', marginTop: 8, marginBottom: 3, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 160px)' }}>
      ...
      <div className="card" style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          {messages.map((m, i) => (
            <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
              ...
              <div style={{
                ...
                background: m.role === 'user' ? 'rgba(184,164,255,0.12)' : 'rgba(255,255,255,0.05)',
                border: `1px solid ${m.role === 'user' ? 'rgba(184,164,255,0.25)' : 'rgba(255,255,255,0.08)'}`,
              }}>
                {m.role === 'user'
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', paddingTop: '14px', marginTop: '14px', display: 'flex', gap: '8px' }}>
          ...
          <button onClick={send} disabled={loading || !input.trim()} style={{ padding: '10px 18px', borderRadius: '10px', background: 'rgba(184,164,255,0.15)', border: '1px solid rgba(184,164,255,0.3)', color: '#B8A4FF', fontWeight: 700, cursor: 'pointer', fontSize: '13px', opacity: loading || !input.trim() ? 0.5 : 1 }}>Send</button>
        </div>
