...
export async function POST(req: NextRequest) {
  try {
    ...
    const { userId, auto } = body as { userId: string; auto?: boolean }
    if (!userId) return NextResponse.json({ error: 'userId required' }, { status: 400 })

    // When called automatically (auto=true), skip if a pattern was updated < 23h ago
    if (auto) {
      const latest = await prisma.behaviorPattern.findFirst({
        where: { userId, autoGenerated: true },
        orderBy: { updatedAt: 'desc' },
      })
      const ageMs = latest ? Date.now() - latest.updatedAt.getTime() : Infinity
      if (ageMs < 23 * 60 * 60 * 1000) {
        return NextResponse.json({ skipped: true, reason: 'analyzed recently' })
      }
      // Delete stale auto-generated patterns before regenerating
      await prisma.behaviorPattern.deleteMany({ where: { userId, autoGenerated: true } })
    }
