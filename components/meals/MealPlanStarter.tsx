...
export default function MealPlanStarter({ userId, weekStart }: Props) {
  ...
  return (
    <div className="card animate-entrance" style={{ textAlign: 'center', padding: '40px 24px' }}>
      ...
      <h2 style={{ fontSize: 18, fontWeight: 700, color: '#FAFAFA', marginBottom: 8 }}>No meal plan for this week</h2>
      <p style={{ fontSize: 14, color: '#B8B6B0', marginBottom: 24, maxWidth: 480, margin: '0 auto 24px' }}>
      ...
      {loading ? (
        <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
          <Spinner size={24} color="#B4A7E5" strokeWidth={2.5} />
          <span style={{ fontSize: 13, color: '#B4A7E5', fontWeight: 600 }}>Generating your meal plan…</span>
          <span style={{ fontSize: 12, color: '#76746E' }}>Building recipes, macros & shopping list</span>
        </div>
      ) : (
        <button
          onClick={generate}
          ...
          style={{
            ...
            color: '#B4A7E5', borderRadius: 8, padding: '12px 28px',
