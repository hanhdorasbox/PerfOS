  ...
  return (
    <div style={{ position: 'relative' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 70px 60px 60px 60px 60px 60px 28px', gap: 4, alignItems: 'center' }}>
        ...
        <button onClick={() => onRemove(index)} style={{ width: 24, height: 24, borderRadius: '50%', background: 'rgba(255,100,100,0.1)', border: '1px solid rgba(255,100,100,0.2)', color: '#FF8A8A', cursor: 'pointer', fontSize: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>✕</button>
      </div>
    </div>
  )
}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {steps.map((s, i) => (
            <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
              ...
              />
              {steps.length > 1 && (
                <button onClick={() => removeStep(i)} style={{ marginTop: 6, width: 24, height: 24, borderRadius: '50%', background: 'rgba(255,100,100,0.08)', border: '1px solid rgba(255,100,100,0.15)', color: '#FF8A8A', cursor: 'pointer', fontSize: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0, flexShrink: 0 }}>✕</button>
              )}
            </div>
          ))}
        </div>
