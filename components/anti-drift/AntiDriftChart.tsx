...
const CATEGORIES = [
  { key: 'advancement', label: 'Advancement', color: '#64f0aa', description: 'Creates lasting value' },
  { key: 'maintenance', label: 'Maintenance', color: '#61adff', description: 'Keeps things running' },
  { key: 'reactive', label: 'Reactive', color: '#ffce53', description: 'Response to external demands' },
  { key: 'busywork', label: 'Busywork', color: '#ff8168', description: 'Low-leverage activity' },
]
  ...
  return (
    <div className="card" style={{ marginBottom: 16 }}>
      ...
      {advancementPct < 30 && total > 1 && (
        <div style={{
          background: 'rgba(255, 129, 104,0.12)',
          border: '1px solid rgba(255, 129, 104,0.3)',
          borderRadius: 8,
          padding: '10px 14px',
          marginBottom: 16,
          fontSize: 13,
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, fontWeight: 700, color: '#64f0aa', marginBottom: 8 }}>
          ...
          <ul style={{ margin: 0, padding: '0 0 0 16px' }}>
            {thisWeekAdvancement.slice(0, 5).map(item => (
              <li key={item.id} style={{ fontSize: 13, color: '#A1A1A6', marginBottom: 4 }}>
                ...
                {item.impact && (
                  <span style={{ color: '#64f0aa', marginLeft: 6, fontSize: 11 }}>→ {item.impact}</span>
                )}
              </li>
            ))}
          </ul>
