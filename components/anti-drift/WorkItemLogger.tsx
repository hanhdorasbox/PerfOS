      ...
      {formOpen && (
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20, padding: '12px', background: 'rgba(255,255,255,0.02)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <input
            ...
            style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '8px 12px', color: '#F5F5F7', fontSize: 13, width: '100%', boxSizing: 'border-box' }}
          />
          {error && <div style={{ fontSize: 12, color: '#ff8168' }}>{error}</div>}
          <button
            type="submit"
            disabled={loading || !title.trim()}
            style={{ background: 'rgba(160, 133, 255,0.15)', border: '1px solid rgba(160, 133, 255,0.3)', color: '#a085ff', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.6 : 1 }}
          >
                    ...
                    {items.slice(0, 8).map(item => (
                      ...
                      >
                        <div style={{ flex: 1 }}>
                          ...
                          {item.impact && <div style={{ fontSize: 11, color: '#64f0aa', marginTop: 1 }}>→ {item.impact}</div>}
                        </div>
                        <button
                          onClick={() => handleDelete(item.id)}
                          style={{ background: 'none', border: 'none', color: '#ff8168', cursor: 'pointer', fontSize: 12, padding: '2px 6px', flexShrink: 0 }}
                        >
                          ✕
                        </button>
                      </div>
