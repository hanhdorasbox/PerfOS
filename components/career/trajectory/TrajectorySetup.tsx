...
export default function TrajectorySetup({ userId }: { userId: string }) {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    borderRadius: 8, padding: '9px 12px', color: '#FAFAFA', fontSize: 13, width: '100%',
  }
  ...
  return (
    <div style={{ maxWidth: 600 }}>
      <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
        {[1, 2, 3].map(s => (
          <div key={s} style={{
            ...
            background: s <= step ? '#B4A7E5' : 'rgba(255,255,255,0.1)',
          }} />
        ))}
      </div>
      ...
      {step === 1 && (
        <div className="card">
          <h3 style={{ fontSize: 16, fontWeight: 600, color: '#FAFAFA', marginBottom: 18 }}>Step 1: Where You Are Now</h3>
          <div style={{ display: 'grid', gap: 14 }}>
            <div>
              <label style={{ color: '#76746E', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>Current Role *</label>
              <input value={currentRole} onChange={e => setCurrentRole(e.target.value)} placeholder="e.g. Data Analyst" style={inputStyle} />
            </div>
            <div>
              <label style={{ color: '#76746E', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>Level / Seniority</label>
            ...
            <div>
              <label style={{ color: '#76746E', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>Key Responsibilities</label>
            ...
            <div>
              <label style={{ color: '#76746E', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>Key Strengths (comma-separated)</label>
      ...
      {step === 2 && (
        <div className="card">
          <h3 style={{ fontSize: 16, fontWeight: 600, color: '#FAFAFA', marginBottom: 18 }}>Step 2: Where You Want to Go</h3>
          <div style={{ display: 'grid', gap: 14 }}>
            <div>
              <label style={{ color: '#76746E', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>Target Path *</label>
            ...
            <div>
              <label style={{ color: '#76746E', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>Target Role Title</label>
            ...
            <div>
              <label style={{ color: '#76746E', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>Time Horizon</label>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
            <button onClick={() => setStep(1)} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: '#B8B6B0', padding: '8px 16px', borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>← Back</button>
            <button
              onClick={analyzeGaps}
      ...
      {step === 3 && (
        <div>
          <div className="card" style={{ marginBottom: 16 }}>
            <h3 style={{ fontSize: 16, fontWeight: 600, color: '#FAFAFA', marginBottom: 4 }}>Step 3: Your Gap Map</h3>
            <p style={{ color: '#B8B6B0', fontSize: 13 }}>
          ...
          {gaps.map((gap, i) => (
            <div key={i} className="card" style={{ marginBottom: 10 }}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 6 }}>
                <span style={{
                  background: 'rgba(180,167,229,0.12)', color: '#B4A7E5',
                  border: '1px solid rgba(180,167,229,0.25)',
                  padding: '2px 8px', borderRadius: 999, fontSize: 11, fontWeight: 700,
                }}>
                  {gap.gapType}
                </span>
                <span style={{ color: gap.priority === 1 ? '#FF6B6B' : gap.priority === 2 ? '#F2C063' : '#76746E', fontSize: 12, fontWeight: 600 }}>
                  {gap.priority === 1 ? 'High' : gap.priority === 2 ? 'Medium' : 'Low'} priority
                </span>
              </div>
              <p style={{ color: '#FAFAFA', fontSize: 14, fontWeight: 600, marginBottom: 4 }}>{gap.title}</p>
              {gap.description && <p style={{ color: '#B8B6B0', fontSize: 13 }}>{gap.description}</p>}
            </div>
          ))}

          <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
            <button onClick={() => setStep(2)} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: '#B8B6B0', padding: '8px 16px', borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>← Back</button>
            <button
              onClick={saveTrajectory}
              disabled={loading}
              style={{
                background: 'rgba(107,227,164,0.15)', border: '1px solid rgba(107,227,164,0.4)',
                color: '#6BE3A4', padding: '8px 20px', borderRadius: 10,
