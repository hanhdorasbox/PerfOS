...
function gapColor(type: string) { return GAP_COLORS[type] ?? '#76746E' }

function ReadinessBar({ score }: { score: number }) {
  const color = score >= 70 ? '#6BE3A4' : score >= 40 ? '#F2C063' : '#FF6B6B'
  return (
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 12 }}>
        <button
          ...
          style={{
            ...
            color: '#FF6B6B', padding: '7px 16px', borderRadius: 8, fontSize: 12, fontWeight: 600,
      ...
      <div className="card" style={{ background: 'rgba(180,167,229,0.07)', border: '1px solid rgba(180,167,229,0.18)', marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ flex: 1 }}>
            <p style={{ color: '#76746E', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>Target Role</p>
            <p style={{ color: '#FAFAFA', fontSize: 20, fontWeight: 700, lineHeight: 1.2 }}>
              {trajectory.targetRoleTitle || trajectory.targetPath.replace(/_/g, ' ')}
            </p>
            <p style={{ color: '#B8B6B0', fontSize: 13, marginTop: 3 }}>
              from <span style={{ color: '#FAFAFA' }}>{trajectory.currentRole}</span>
              {trajectory.timeHorizon && <> · <span style={{ color: '#B4A7E5' }}>{trajectory.timeHorizon}</span></>}
            </p>
          </div>
          <div style={{ textAlign: 'right', minWidth: 80 }}>
            <p style={{ color: '#76746E', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 3 }}>Open gaps</p>
            <p style={{ color: '#F2C063', fontSize: 22, fontWeight: 700 }}>{openGaps.length}</p>
            <p style={{ color: '#6BE3A4', fontSize: 12, marginTop: 1 }}>{closedGaps.length} closed</p>
          </div>
        </div>
        ...
        {readinessScore !== null && (
          <div style={{ marginTop: 16, paddingTop: 14, borderTop: '1px solid rgba(255,255,255,0.07)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <p style={{ color: '#B8B6B0', fontSize: 12, fontWeight: 600 }}>Readiness Score</p>
              <button
                onClick={generateRoadmap}
                disabled={generatingRoadmap}
                style={{ background: 'none', border: '1px solid rgba(180,167,229,0.3)', color: '#B4A7E5', borderRadius: 6, padding: '3px 10px', fontSize: 11, cursor: generatingRoadmap ? 'not-allowed' : 'pointer' }}
              >
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 10 }}>
                {Object.entries(readinessBreakdown).map(([key, val]) => (
                  <div key={key} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                    ...
                    <span style={{ color: '#76746E', fontSize: 10, textTransform: 'capitalize' }}>{key.replace(/_/g, ' ')}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {trajectoryNextAction && (
          <div style={{ marginTop: 12, background: 'rgba(107,227,164,0.08)', border: '1px solid rgba(107,227,164,0.2)', borderRadius: 10, padding: '10px 14px' }}>
            <p style={{ color: '#6BE3A4', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 3 }}>Next Best Action</p>
            <p style={{ color: '#FAFAFA', fontSize: 13 }}>{trajectoryNextAction}</p>
          </div>
        )}
        ...
        {readinessScore === null && (
          <button
            ...
            style={{
              ...
              color: '#B4A7E5', padding: '10px 0', borderRadius: 10, fontSize: 13, fontWeight: 600,
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <h3 style={{ fontSize: 16, fontWeight: 600, color: '#FAFAFA', marginBottom: 16 }}>Gap Execution Plan</h3>
                ...
                >
                  ...
                  <div style={{ padding: '12px 14px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 7 }}>
                          ...
                          <span style={{
                            color: gap.priority === 1 ? '#FF6B6B' : gap.priority === 2 ? '#F2C063' : '#76746E',
                            fontSize: 10, fontWeight: 700,
                          }}>
                            {gap.priority === 1 ? '🔴 High' : gap.priority === 2 ? '🟡 Med' : '🟢 Low'}
                          </span>
                          {gap.difficulty && (
                            <span style={{ color: DIFF_COLORS[gap.difficulty] ?? '#76746E', fontSize: 10, fontWeight: 700 }}>
                              {gap.difficulty}
                            </span>
                          )}
                          {gap.weekEstimate && (
                            <span style={{ color: '#76746E', fontSize: 10 }}>~{gap.weekEstimate}w</span>
                          )}
                        </div>

                        <p style={{ color: '#FAFAFA', fontSize: 14, fontWeight: 600, marginBottom: 3 }}>{gap.title}</p>
                        {gap.description && (
                          <p style={{ color: '#76746E', fontSize: 12, lineHeight: 1.5 }}>{gap.description}</p>
                        )}
                        {gap._whyItMatters && (
                          <p style={{ color: '#B4A7E5', fontSize: 12, marginTop: 5, fontStyle: 'italic' }}>{gap._whyItMatters}</p>
                        )}
                      </div>
                    </div>
                    ...
                    {gap.nextBestAction && (
                      <div style={{ background: 'rgba(107,227,164,0.1)', border: '1px solid rgba(107,227,164,0.2)', borderRadius: 8, padding: '7px 11px', marginTop: 8 }}>
                        <p style={{ color: '#6BE3A4', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 2 }}>Next Action</p>
                        <p style={{ color: '#FAFAFA', fontSize: 12 }}>{gap.nextBestAction}</p>
                      </div>
                    )}
                    ...
                    {gap.evidenceNeeded && (
                      <div style={{ marginTop: 6 }}>
                        <span style={{ color: '#76746E', fontSize: 11 }}>
                          <span style={{ color: '#B8B6B0', fontWeight: 600 }}>Done when: </span>{gap.evidenceNeeded}
                        </span>
                      </div>
                    )}
                    ...
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 12 }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          color: '#6BE3A4', padding: '5px 12px', borderRadius: 8,
                      ...
                      <button
                        ...
                        style={{
                          ...
                          color: '#76746E', padding: '5px 10px', borderRadius: 8, fontSize: 11, cursor: 'pointer',
                        }}
                      >
                        Archive
                      </button>
                      <button
                        ...
                        style={{
                          ...
                          color: '#FF6B6B', padding: '5px 10px', borderRadius: 8, fontSize: 11, cursor: 'pointer',
                        }}
                      >
                        {isDeleting ? '…' : 'Delete'}
                      </button>
                  ...
                  {isExpanded && steps.length > 0 && (
                    <div style={{ borderTop: `1px solid ${color}20`, padding: '12px 14px' }}>
                      <p style={{ color: '#76746E', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>
                          ...
                          <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                            ...
                            <div style={{ flex: 1 }}>
                              <div style={{ display: 'flex', gap: 8, alignItems: 'baseline', flexWrap: 'wrap' }}>
                                <span style={{ color: '#FAFAFA', fontSize: 13 }}>{step.action}</span>
                                <span style={{ color: '#76746E', fontSize: 11 }}>{step.timeframe}</span>
                              </div>
                              <span style={{ color: '#76746E', fontSize: 11 }}>→ {step.output}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                      <div style={{ marginTop: 12, display: 'flex', gap: 6 }}>
                        <button
                          ...
                          disabled={isGenerating}
                          style={{
                            background: 'none', border: `1px solid ${color}30`, color: '#76746E',
      ...
      {(roadmap.length > 0 || generatingRoadmap) && (
        <div className="card" style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <h3 style={{ fontSize: 16, fontWeight: 600, color: '#FAFAFA' }}>Execution Roadmap</h3>
            <button
                ...
                <div key={i} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      <span style={{ color: '#B4A7E5', fontSize: 12, fontWeight: 700 }}>{phase.phase}</span>
                      <span style={{ color: '#76746E', fontSize: 11 }}>{phase.monthRange}</span>
                    </div>
                  </div>
                  <p style={{ color: '#FAFAFA', fontSize: 13, fontWeight: 600, marginBottom: 6 }}>{phase.focus}</p>
                  {phase.milestones?.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 3, marginBottom: 6 }}>
                      {phase.milestones.map((m, j) => (
                        <div key={j} style={{ display: 'flex', gap: 7, alignItems: 'flex-start' }}>
                          <span style={{ color: '#B4A7E5', fontSize: 11, marginTop: 1 }}>·</span>
                          <span style={{ color: '#B8B6B0', fontSize: 12 }}>{m}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  <div style={{ background: 'rgba(107,227,164,0.07)', borderRadius: 7, padding: '6px 10px' }}>
                    <span style={{ color: '#6BE3A4', fontSize: 11, fontWeight: 600 }}>Output: </span>
                    <span style={{ color: '#FAFAFA', fontSize: 12 }}>{phase.keyOutput}</span>
                  </div>
                  {phase.gapsClosed && phase.gapsClosed.length > 0 && (
                    <div style={{ marginTop: 6, display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                      {phase.gapsClosed.map((g, j) => (
                        <span key={j} style={{ color: '#76746E', fontSize: 10, background: 'rgba(255,255,255,0.04)', padding: '2px 7px', borderRadius: 99, border: '1px solid rgba(255,255,255,0.08)' }}>
                          closes: {g}
                        </span>
                      ))}
                    </div>
      ...
      {suggestedProjects.length > 0 && (
        <div className="card" style={{ marginBottom: 16 }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, color: '#FAFAFA', marginBottom: 14 }}>Suggested Projects</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {suggestedProjects.map((p, i) => (
              <div key={i} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: '12px 14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 5 }}>
                  <p style={{ color: '#FAFAFA', fontSize: 13, fontWeight: 600 }}>{p.title}</p>
                  <span style={{ color: '#76746E', fontSize: 11, whiteSpace: 'nowrap', marginLeft: 8 }}>{p.effort}</span>
                </div>
                <p style={{ color: '#B8B6B0', fontSize: 12, marginBottom: 6 }}>{p.why}</p>
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                  ...
                  <span style={{ color: '#6BE3A4', fontSize: 11 }}>→ {p.impact}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, color: '#FAFAFA' }}>Quarterly Focus</h3>
          <button
          ...
          <div>
            ...
            {latestPlan.keyOutput && (
              <div style={{ background: 'rgba(107,227,164,0.08)', border: '1px solid rgba(107,227,164,0.2)', borderRadius: 10, padding: '10px 14px', marginBottom: 10 }}>
                <p style={{ color: '#6BE3A4', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>Key Output</p>
      ...
      {closedGaps.length > 0 && (
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          >
            <h3 style={{ fontSize: 14, fontWeight: 600, color: '#6BE3A4' }}>✓ Closed Gaps ({closedGaps.length})</h3>
            <span style={{ color: '#76746E', fontSize: 12 }}>{showClosed ? '▲' : '▼'}</span>
          </button>
          {showClosed && (
            <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
              {closedGaps.map(gap => (
                <div key={gap.id} style={{
                  ...
                  <div>
                    <span style={{ color: '#6BE3A4', fontSize: 12, fontWeight: 600 }}>{gap.title}</span>
                    <span style={{ color: '#76746E', fontSize: 11, marginLeft: 8 }}>
                      {gap.gapType.replace(/_/g, ' ')}
                    </span>
                  </div>
                  <button
                    onClick={() => reopenGap(gap.id)}
                    style={{ background: 'none', border: '1px solid rgba(255,255,255,0.1)', color: '#76746E', borderRadius: 6, padding: '3px 8px', fontSize: 11, cursor: 'pointer' }}
                  >
                    Reopen
                  </button>
                </div>
      ...
      {archivedGaps.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <button
            ...
            style={{ background: 'none', border: 'none', color: '#76746E', fontSize: 12, cursor: 'pointer', padding: 0 }}
          >
            ...
            <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
              {archivedGaps.map(gap => (
                <div key={gap.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 12px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 8 }}>
                  <span style={{ color: '#76746E', fontSize: 12 }}>{gap.title}</span>
                  <button
