...
export default function PatternAnalyzer({ userId }: Props) {
  ...
  return (
    <div>
      ...
      {saved && (
        <p style={{ color: '#6BE3A4', fontSize: 13, marginTop: 8 }}>Patterns saved successfully!</p>
      )}

      {patterns.length > 0 && (
        <div style={{ marginTop: 20 }}>
          <div className="card" style={{ marginBottom: 12 }}>
            <p style={{ color: '#FAFAFA', fontWeight: 600, fontSize: 15, marginBottom: 4 }}>
              {patterns.length} new pattern{patterns.length !== 1 ? 's' : ''} identified
            </p>
            <p style={{ color: '#B8B6B0', fontSize: 13 }}>Review and confirm to save.</p>
          </div>
          ...
          {patterns.map((p, i) => (
            <div key={i} className="card" style={{ marginBottom: 10 }}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
                <span style={{
                  background: `${domainColors[p.domain] ?? '#76746E'}20`,
                  color: domainColors[p.domain] ?? '#76746E',
                  border: `1px solid ${domainColors[p.domain] ?? '#76746E'}40`,
                  padding: '2px 10px', borderRadius: 999, fontSize: 11, fontWeight: 700,
                }}>
                  {p.domain}
                </span>
                <span style={{ color: '#76746E', fontSize: 12 }}>Confidence: {p.confidence}/5</span>
              </div>
              <p style={{ color: '#FAFAFA', fontSize: 14, marginBottom: 6 }}>{p.pattern}</p>
              {p.evidence && <p style={{ color: '#B8B6B0', fontSize: 12, marginBottom: 4 }}>Evidence: {p.evidence}</p>}
              {p.implication && (
                <p style={{ color: '#B4A7E5', fontSize: 12 }}>→ {p.implication}</p>
              )}
            </div>
          ))}
